...
class HetNet(nn.Module):
    def __init__(self, gnn, num_feature, num_embed, node_count, emb_dim, n_heads,
        ...
        self.num_node_type = 6 #different types of nodes
    ...
    def create_time_embedds(self):
        ...
        self.d_freq = nn.Embedding(self.numNode, self.t_emb_dim)
        ...
        self.d_phi = nn.Embedding(self.numNode, self.t_emb_dim)
        ...
        self.d_amp = nn.Embedding(self.numNode, self.t_emb_dim)
    ...
    def forward(self, x, edge, heads, rels, tails, weeks, days, node_type, edge_type, edge_index=None, **kwargs):
        ...
        for src, edge_grp in tqdm.tqdm(edge.groupby('src'), position=0, mininterval=20):
          emb[src,:] = self.lstmmodel(scores[edge_grp.index].view(-1, scores[edge_grp.index].shape[0], self.emb_dim))

        for dst, edge_grp in tqdm.tqdm(edge.groupby('dst'), position=0, mininterval=20):
