...
# ebay exp
from glib.pyg.model_DE import HetNet as Net, GNN
...
def get_data(y_selected_columns=('gmv_class', 'risky'), debug=False,
             ts_snapshot=False, gmv_fair=False, get_node_type=True):
    """

    Parameters
    ----------
    y_selected_columns : TYPE, optional
        DESCRIPTION. The default is ('gmv_class', 'risky').
    debug : TYPE, optional
        DESCRIPTION. The default is False.
    ts_snapshot : BOOL, optional
        DESCRIPTION. The default is False.
    gmv_fair : BOOL, optional
        DESCRIPTION. The default is False.
    ...
    g : TYPE
        DESCRIPTION.
    x : dataframe
        node_id, features, ts, ts_weekn phase
    y : array
        y[y_selected_columns]
    ...
    path_node = f'{DIR_DATA}/node.parquet'
    ...
    if ts_snapshot:
        ...
        for dst_origin, edge_grp in tqdm.tqdm(
                ...
                total=edge_new['dst_origin'].nunique(), desc='ts-edge-init', position=0, leave=True):
...
def main(path_result='result-mass-reg.csv',
         ...
         seed=2020, verbose=False, gmv_fair=False, debug=False):

    np.random.seed(seed)
    torch.manual_seed(seed)
    
    results_file = 'results_lstm_emb60_'+str(seed)+'.txt'
    ...
    stats = dict(
        ...
        max_epochs=max_epochs, patience=patience, emb_dim=emb_dim,
        seed=seed, gmv_fair=gmv_fair
    )
    logger.info('Param %s', stats)
    
    g, x, y, g_ts, edge, node_type = get_data(ts_snapshot=dysat, gmv_fair=gmv_fair, debug=debug, get_node_type=False)
    print(x.shape, y.shape)
    node_count = len(np.unique(g[:, 0]))+len(np.unique(g[:, 1]))
    edge_count = g.shape[0]
    
    path_feat = f'{DIR_DATA}/feat.parquet'
    feat = pd.read_parquet(path_feat)
    ...
    model = Net(
        gnn, num_feature=num_feat+emb_dim,
    ...
    """
    mask_train_origin_idx = x.index[mask_train.cpu()].tolist()
    ...
    """
    #train/validation split is done chronologically instead of randomly, we choose ts = 58 to have a 80/20 split
    mask_train = (y0>=0) & convert_tensor(torch.BoolTensor((x['phase'] == 'TRAIN')), device=device) & convert_tensor(torch.BoolTensor((x['ts'] <= 58)), device=device)
    mask_valid = (y0>=0) & convert_tensor(torch.BoolTensor((x['phase'] == 'TRAIN')), device=device) & convert_tensor(torch.BoolTensor((x['ts'] > 58)), device=device)
    ...
    heads = torch.tensor(edge["src"]).long().to(device)
    rels = torch.tensor(edge["edge_type"]-1).long().to(device) #-1 to get all the values between 0 and 3
    tails = torch.tensor(edge["dst"]).long().to(device)
    weeks = torch.tensor(edge["ts_wk"]).float().to(device)
    days = torch.tensor(edge["ts"]).float().to(device)
    
    edge_type = torch.cat((torch.tensor(edge["edge_type"]), torch.tensor(edge["edge_type"]))).to(device) #to match edge_index
    ...
    def train(#loss0_fn=nn.SmoothL1Loss(),
        ...
        out = model((mask_train, x, edge_index, edge_index_ts), edge, heads, rels, tails, weeks, days)
        y0_yhat = out[:, :5]
        y1_yhat = out[:, 5]
...
#       loss = 0.33 * loss0 + 0.66 * loss1
    ...
    def test(epoch, mask):
        model.eval()
        out = model((mask, x, edge_index, edge_index_ts), edge, heads, rels, tails, weeks, days)
        y0_yhat = out[:, :5]
        y1_yhat = out[:, 5]
    ...
    for epoch in range(1, max_epochs+1):
        ...
        pbar.set_description(f'train-stg0 epoch {epoch} - gmv accuracy {m0["accuracy"]:.4f} gmv auc {m0["auc"]:.4f}  binary auc {m1["auc"]:.4f} ap {m1["ap"]:.4f}')
        
        with open(results_file, 'a') as f:
            f.write(f'train-stg0 epoch {epoch} - gmv accuracy {m0["accuracy"]:.4f} gmv auc {m0["auc"]:.4f}  binary auc {m1["auc"]:.4f} ap {m1["ap"]:.4f}')
            f.write('\n')
        if metric > best[0]:
            best = metric, epoch
    ...
    for epoch in range(1, best[-1]+1):
        ...
        pbar.set_description(f'train-stg1 epoch {epoch} - gmv accuracy {m0["accuracy"]:.4f} gmv auc {m0["auc"]:.4f}  binary auc {m1["auc"]:.4f} ap {m1["ap"]:.4f}')
        with open(results_file, 'a') as f:
            f.write(f'train-stg1 epoch {epoch} - gmv accuracy {m0["accuracy"]:.4f} gmv auc {m0["auc"]:.4f}  binary auc {m1["auc"]:.4f} ap {m1["ap"]:.4f}')
            f.write('\n')
        stats['gmv_accuracy'] = m0['accuracy']
        stats['gmv_auc'] = m0['auc']
        for k, v in m1.items():
        ...
        # Uncomment here to save the embedding
        """
        if (epoch-1)%50==0:
            #model.saveEmbedding(x, epoch, edge, heads, rels, weeks, days, "stg1")
            model.saveEmbedding2(epoch, "stg1")
        """    
