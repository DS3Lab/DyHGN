...
def main(path_result='result-mass-reg.csv',
    ...
    results_file = 'results_lstm_hgt'+str(seed)+'.txt'
    ...
    g, x, y, g_ts, edge, node_type = get_data(ts_snapshot=dysat, gmv_fair=gmv_fair, debug=debug, get_node_type=True)
    ...
    def train(#loss0_fn=nn.SmoothL1Loss(),
        ...
        out = model((mask_train, x, edge_index, edge_index_ts), edge, heads, rels, tails, weeks, days, node_type, edge_type)
        y0_yhat = out[:, :5]
        y1_yhat = out[:, 5]
...
#       loss = 0.33 * loss0 + 0.66 * loss1
    ...
    def test(epoch, mask):
        model.eval()
        out = model((mask, x, edge_index, edge_index_ts), edge, heads, rels, tails, weeks, days, node_type, edge_type)
        y0_yhat = out[:, :5]
        y1_yhat = out[:, 5]
    ...
    for epoch in range(1, max_epochs+1):
        ...
        if epoch - best[-1] > patience:
            break
        scheduler.step()
        # Uncomment to save embedding
        """
        #save embedding to visualize during training
        if (epoch-1)%50==0:
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=128,
        num_training_steps=max_epochs+1)
    ...
    pbar = tqdm.tqdm(desc='train-stg1', total=max_epochs)
    for epoch in range(1, max_epochs+1):
        pbar.update(1)
        train()
